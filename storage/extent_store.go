...
func (s *ExtentStore) MkdirAll(name string) (err error) {
	if s.ExtentFs.FsType == ExtentFsStandard {
...
func NewExtentStore(fs *ExtentFs, dataDir string, partitionID uint64, storeSize int) (s *ExtentStore, err error) {
	...
	if err = s.MkdirAll(dataDir); err != nil {
...
func (s *ExtentStore) initBlobFSBaseFileID(baseFileID uint64) (err error) {
	if s.FsType != ExtentFsBlobFS {
		return s.initBaseFileID()
	}
	...
	files, err := s.NvmeDev.GetFilesNameWithPrefix(prefix)
	if err != nil {
		return err
	}
...
func (s *ExtentStore) initBaseFileID() (err error) {
	...
	if s.FsType == ExtentFsBlobFS {
		return s.initBlobFSBaseFileID(baseFileID)
	}
...
func (s *ExtentStore) GetStoreUsedSize() (used int64) {
	...
	for _, einfo := range extentInfoSlice {
		...
		if IsTinyExtent(einfo.FileID) {
			filePath := fmt.Sprintf("%s/%d", s.dataPath, einfo.FileID)
			if s.FsType == ExtentFsStandard {
			...
			} else if s.FsType == ExtentFsBlobFS {
				if size, err := s.NvmeDev.FileSize(filePath); err == nil {
					used += size
				}
